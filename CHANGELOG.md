...
- modules/standard/restore_point: Post-Apply Verification を追加(-Verified)。24h-limit は適用後に既存
  ...
  0→1 にする)で直接検証し、冪等性チェックも同指標へ修正(従来の DisableSR=0 は modern Windows で常時 ABSENT な
  死指標〔clean=0/disabled=0/enabled=1 を VM 実測〕で skip が一度も効いていなかった既存バグを解消)。
  set_storage_size のみ vssadmin parse 脆弱で対象外(success/fail 計上は従来通り)。
...
- modules/extended/dpi_config: Post-Apply Verification を追加(-Verified)。Write-DpiValue 内で HKCU /
  ...
  VM 検証で Get-CurrentDpiValue の符号付き DWORD バグ(PS5.1 が 0xFFFFFFFF を 4294967295 と読み [int]
  オーバーフロー→null 返却)を発見・修正(負の DPI オフセットで読返・冪等性が壊れていた既存バグ)。
...
- apps/fabriq_backuper v0.9.0 → v0.9.1 (Phase 2.9.0a / outlook_pop value 名修正 + SectionParam 配線):
  ...
  - Phase A 実機検証ログ:
    - 1 POP アカウント (`user@example.jp` @ pop.example.jp:995 SSL) を
      正常採取
