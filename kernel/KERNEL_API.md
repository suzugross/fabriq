...
**Current Kernel Version**: `3.6.2`（`kernel/KERNEL_VERSION` を真のソースとする。CLAUDE.md ルール J step 3 で sync 対象）
...
| `__GATE__` | 前進バリア（**since kernel 3.6.0**）。直前ゲート〜本マーカの窓に `Error`/`Partial` または **Post-Apply Verification 失敗（`Verified=False`）** のモジュールが残る間、本マーカ以降の Order の実行を `Invoke-BatchExecution` が拒否する（動的評価）。FlexProfile では該当行をグレーアウト。窓が解消すると解除。`Verified=$null`（検証非対応）/`Pending`（未実行）はブロックしない（失敗を防ぐが省略は防がない） |
...
- `Get-FabriqModulesRoot` / `Get-FabriqProfileDataDir` / `Set-FabriqProfileDataContext` / `Clear-FabriqProfileDataContext` / `Test-FabriqResumeDataDir` / `Get-FabriqOverlayCandidate` / `Write-FabriqDataResolution` / `Get-FabriqDataOrigin`（プロファイル別データオーバーレイの内部機構 — コンテキスト導出・写像・採用元表示・テレメトリ分類。モジュールが依存してよい公開面は §1.2 の `Resolve-ModuleDataPath` / `Get-ModuleDataFiles` と §3.2 の `FABRIQ_PROFILE_DATA_DIR` のみ）
...
- **§1.2 `Get-ModuleDataFiles` 追加 + `Resolve-ModuleDataPath` のディレクトリ対応（後方互換 / MINOR）**:
  プロファイル別データオーバーレイ Phase 2（同計画書 §12）。資材フォルダ（driver/ certs/ xml/ file/
  source/ INF/ prompt/ 等）を**フォルダ単位 all-or-nothing** で解決し、`reg_hklm` / `reg_hkcu` 系の
  複数 CSV 列挙を**モジュール単位 all-or-nothing** で解決する。ディレクトリ対応は「PDF 側にフォルダが
  存在する」ときだけ効くため、コンテキスト無し・PDF にフォルダ無しは Phase 1 と同一の恒等写像。
...
- **§4 特殊マーカーに `__GATE__` 追加（後方互換 / MINOR）**: 前進バリア（TM t-0073）
  - `Invoke-BatchExecution` が、直前ゲート〜当該 `__GATE__` の窓に `Error`/`Partial` または **Post-Apply Verification 失敗（`Verified=False`）** を持つモジュールがある間、当該マーカ以降の `Order` の実行を拒否する（admission control）。判定は各モジュール実行直前に live な状態（当該 run の蓄積 + session history）で動的に行うため、フォワード実行中に発生した失敗もゲートで止まる。`Success`（かつ `Verified` が `True`/`$null`）/`Skipped`/`Cancelled`/`Pending` はブロックしない。`Verified=False` は「適用は成功扱いだが読み返し検証が不一致＝設定が効いていない」を意味し、下流依存を守る趣旨からブロック対象に含む
...
|---|---|---|
| **kernel** | `kernel/KERNEL_VERSION` | `kernel/`, `apps/`, `commands/`, `dev/`, `tests/`, `Fabriq.exe`, `README.md`, `CHANGELOG.md`, `CLAUDE.md`, `LICENSE` |
