...
- **メニューからの単発実行は常にモジュール本体の CSV を使います**（オーバーレイはプロファイル実行だけに効く仕様です）。
- **backup / export 系の出力先**もプロファイル側です。ACL・レジストリ・ファイアウォールルール・デスクトップアイコン・ドライバ・既定アプリの各バックアップ／エクスポートは、プロファイル実行中なら `profiles/<name>/modules/<module>/backup/` 等に書かれ、対応する restore／import がそこから読みます（**ある案件の捕捉物が別案件に混ざりません**）。復元側でプロファイルにバックアップが無い場合はモジュール本体側にフォールバックし、その旨が警告表示されます。CSV に絶対パスで出力先を指定した場合（`firewall_rule_config` の `DestinationPath` 等）は指定先がそのまま使われます。
- 読み書きが同じフォルダで閉じる族（`startlayout_config` の json/xml/ppkg、`sysprep_config` の source と `taskbar_config` の書き込み、`printer_driver_config` の INF 展開）は、**プロファイル側にフォルダを作っておけば入出力ともそこに閉じます**。
- backup と restore は**同じプロファイルの中で対にして**実行してください。メニューからの単発実行はオーバーレイが効かないため、プロファイル側に取ったバックアップは見えません。
