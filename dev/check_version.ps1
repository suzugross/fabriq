...
# Search-based check for version strings that sit MID-FILE (line numbers
# drift with every edit). Added 2026-06-11 after the startup banner and
# the HTML checklist footer were found 2 major versions stale (ver2.1
# at kernel 3.4.1) - neither was covered by the line-number checks above.
function Test-VersionPatternAnywhere {
    param(
        [string]$Label,
        [string]$FilePath,
        [string]$Pattern,
    ...
    $hit = Select-String -Path $FilePath -Pattern $Pattern | Select-Object -First 1
    if ($null -eq $hit) {
        Write-Host "[FAIL] $Label : pattern not found anywhere in file" -ForegroundColor Red
        $script:mismatches += $Label
        return
    }
    if ($hit.Line -match $Pattern) {
        $actual = $Matches[1]
        if ($actual -eq $Expected) {
            Write-Host "[ OK ] $Label : $actual (line $($hit.LineNumber))" -ForegroundColor Green
        } else {
            Write-Host "[FAIL] $Label : expected '$Expected', got '$actual' (line $($hit.LineNumber))" -ForegroundColor Red
            $script:mismatches += $Label
        }
    }
}

# kernel/main.ps1 startup banner : Write-Host "Fabriq ver{X.Y} ..."
Test-VersionPatternAnywhere `
    -Label "kernel/main.ps1 startup banner" `
    -FilePath (Join-Path $projectRoot "kernel\main.ps1") `
    -Pattern 'Write-Host "Fabriq ver(\d+\.\d+)' `
    -Expected $majorMinor

# kernel/common.ps1 HTML checklist footer : "Generated by Fabriq ver{X.Y}"
Test-VersionPatternAnywhere `
    -Label "kernel/common.ps1 HTML footer" `
    -FilePath (Join-Path $projectRoot "kernel\common.ps1") `
    -Pattern 'Generated by Fabriq ver(\d+\.\d+)' `
    -Expected $majorMinor
