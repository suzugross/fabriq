# ========================================
# Fabriq BackUper Launcher Build Script
# Compiles Launcher_BackUper.cs into Fabriq_BackUper.exe using csc.exe.
# Produces the final binary at the fabriq root: ..\..\Fabriq_BackUper.exe
# Reuses fabriq.ico (no separate icon for now).
# ========================================
...
Show-Separator
Show-CategorySeparator "Fabriq BackUper Launcher Build"
Show-Separator
...
# Reuse existing fabriq.ico for now.
...
$sourceCs   = Join-Path $scriptDir 'Launcher_BackUper.cs'
$manifest   = Join-Path $scriptDir 'app_backuper.manifest'
$outputExe  = Join-Path $fabriqRoot 'Fabriq_BackUper.exe'
...
$running = Get-Process -Name 'Fabriq_BackUper' -ErrorAction SilentlyContinue
if ($running) {
    Show-Warning "Fabriq_BackUper.exe is currently running. Terminate it before rebuild."
    if (-not $Force) {
...
Show-Info "Compiling Launcher_BackUper.cs -> Fabriq_BackUper.exe ..."
