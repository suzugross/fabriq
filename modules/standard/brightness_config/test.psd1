...
@{
    schema = 1
    module = 'brightness_config'
    category = 'A-interactive'
    scenarios = @(
        @{
            name = 'apply'; script = 'brightness_config.ps1'
            ...
            oracle = @{ type = 'self-verified' }   # WMI brightness; absent on a plain VM => Skipped
            idempotent = @{ secondRun = 'Skipped' }
            cleanup = 'none'
            notes = 'WMI display brightness. Bare VM usually lacks a controllable display => Skipped.'
        }
    )
}
